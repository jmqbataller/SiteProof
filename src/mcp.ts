...
export function createSiteProofMcp(){
  ...
  server.registerTool('quickcheck_site',{title:'SiteProof five-point QuickCheck',description:'Check public DNS, website functionality, known subdomains, email DNS and SSL/HTTPS from one website URL. Private account and delivery facts remain access-dependent.',annotations:readOnly,inputSchema:z.object({url:z.string().url(),subdomains:z.array(z.string()).max(20).optional(),dkimSelectors:z.array(z.string().regex(/^[a-z0-9][a-z0-9._-]{0,62}$/i)).max(10).optional()})},async({url,subdomains,dkimSelectors})=>{const out=await quickCheckSite(url,{subdomains,dkimSelectors});return {content:[{type:'text',text:JSON.stringify(out)}],structuredContent:out};});
